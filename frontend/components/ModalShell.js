...
export default function ModalShell({ open, title, icon: Icon, color = "#6366f1", glow = "glow-blue", onClose, children }) {
  ...
  return (
    <AnimatePresence>
      {open ? (
        <motion.div className="modal-backdrop fixed inset-0 z-50 flex items-center justify-center p-5 backdrop-blur-md" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          <motion.section
          ...
          >
            <header className="theme-border-soft flex min-h-[100px] items-center justify-between border-b px-7">
              <div className="flex items-center gap-5">
                ...
                <div>
                  ...
                  <p className="theme-text-muted mt-2 text-[14px]">Manage and analyze digital marketing activity in one place.</p>
                </div>
              </div>
              <button className="modal-close flex h-10 w-10 items-center justify-center rounded-[10px] transition" onClick={onClose} type="button" aria-label="Close modal">
                <X size={24} />
              </button>
            </header>
