      ...
      api
        ...
        .then((res) => {
          ...
          const filtered = list.filter(
            (u) => u.id !== currentUser?.id && u.empCode !== currentUser?.code
          );
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/75 backdrop-blur-sm p-4 animate-in fade-in duration-200">
        <motion.div
          ...
          className="employee-start-chat-modal bg-slate-900 border border-slate-700/80 rounded-3xl shadow-2xl w-full max-w-xl max-h-[85vh] flex flex-col overflow-hidden text-slate-100 relative p-6"
        >
          {/* Header */}
          ...
          {/* Search Box */}
          <div className="employee-start-chat-search relative mt-5 mb-4">
            ...
            {isLoading ? (
              ...
              filteredUsers.map((user) => (
                <div
                  ...
                  className="employee-start-chat-user-card p-3.5 rounded-2xl bg-slate-800/40 hover:bg-slate-800 border border-slate-800 hover:border-slate-700 flex items-center justify-between gap-4 transition-all cursor-pointer group hover:scale-[1.01]"
                >
                  ...
                  <div className="w-8 h-8 rounded-full bg-violet-100 border border-violet-200 text-violet-600 group-hover:bg-violet-600 group-hover:text-white flex items-center justify-center transition-all shrink-0 shadow-sm dark:bg-slate-700/40 dark:border-transparent dark:text-slate-400 dark:group-hover:bg-blue-600 dark:group-hover:text-white">
  <ArrowRight size={16} />
</div>
                </div>
              ))
