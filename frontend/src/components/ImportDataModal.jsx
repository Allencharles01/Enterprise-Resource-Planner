...
export default function ImportDataModal({ onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-md px-4">
      <div className="w-full max-w-6xl max-h-[90vh] overflow-y-auto rounded-2xl border border-primary/30 bg-[#090d1f] shadow-2xl shadow-primary/20">
        <div className="sticky top-0 z-20 flex items-start justify-between border-b border-border bg-[#090d1f]/95 px-7 py-5 backdrop-blur-xl">
          <div className="flex items-center gap-4">
            <div className="flex h-11 w-11 items-center justify-center rounded-xl border border-violet-500/30 bg-violet-500/15 text-violet-400">
              <Upload size={22} />
            </div>
        ...
        <div className="space-y-6 p-7">
          {!selectedFile ? (
            <label className="flex min-h-[230px] cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed border-violet-500/30 bg-violet-500/5 p-8 text-center transition hover:bg-violet-500/10">
              <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-2xl border border-violet-500/30 bg-violet-500/15 text-violet-400">
                <FileText size={28} />
              </div>
              ...
              <div className="mt-4 flex items-center gap-2">
                {["CSV", "PDF", "DOC", "DOCX"].map((type) => (
                  <span
                    key={type}
                    className="rounded-md border border-violet-500/30 bg-violet-500/10 px-3 py-1 text-xs font-bold text-violet-300"
                  >
                    {type}
                  </span>
                ))}
            ...
            <div className="flex items-center justify-between rounded-2xl border border-violet-500/30 bg-violet-500/10 p-5">
              <div className="flex items-center gap-4">
              ...
              <button
                onClick={removeFile}
                className="rounded-xl p-2 text-muted-foreground transition hover:bg-rose-500/10 hover:text-rose-400"
              >
                <Trash2 size={18} />
              </button>
            </div>
      ...
      {showSuccessPopup && (
        <SmallPopup title="Successfully Assigned" onClose={onClose}>
          <div className="flex flex-col items-center text-center">
            ...
            <button
              onClick={onClose}
              className="mt-6 w-full rounded-xl bg-emerald-500 px-4 py-2 text-sm font-bold text-white hover:bg-emerald-600"
            >
              Close
            </button>
          </div>
  ...
  return (
    <div className="rounded-2xl border border-border bg-[#10142b] p-5">
            ...
            <thead>
              <tr className="text-muted-foreground">
                ...
                {headers.map((header) => (
                  <th
                    key={header}
                    className="sticky top-0 z-20 min-w-[220px] whitespace-nowrap border-b border-border bg-[#090d1f] px-5 py-4 font-bold"
                  >
                    {header}
                  </th>
                ))}
                  ...
                  <tr key={record.id} className="hover:bg-muted/20">
                    ...
                    {headers.map((header) => (
                      <td
                        ...
                        className="whitespace-nowrap border-b border-border/50 px-5 py-4 text-muted-foreground"
                      >
                        {record[header] || "—"}
                      </td>
                    ))}
      ...
      <div className="mt-5 grid grid-cols-1 gap-4 md:grid-cols-2">
        <button
          onClick={onSync}
          className="flex items-center justify-center gap-2 rounded-2xl border border-cyan-500/30 bg-cyan-500/10 px-5 py-3 text-sm font-bold text-cyan-400 transition hover:bg-cyan-500/20"
        >
          <RefreshCw size={16} />
          Sync Exact Entries
        </button>
        ...
        <button
          onClick={onCompare}
          className="flex items-center justify-center gap-2 rounded-2xl border border-violet-500/30 bg-violet-500/10 px-5 py-3 text-sm font-bold text-violet-300 transition hover:bg-violet-500/20"
        >
          <GitCompare size={16} />
          Compare Conflicts
        </button>
...
function AssignSection({
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-border bg-[#10142b]">
      ...
      <div className="space-y-4 p-5">
        <div className="flex flex-col gap-3 lg:flex-row">
          <div className="flex flex-1 items-center gap-3 rounded-2xl border border-border bg-background/60 px-4 py-3">
          ...
          <div className="flex overflow-hidden rounded-2xl border border-border bg-background/60">
        ...
        <div className="max-h-[280px] overflow-auto rounded-2xl border border-border">
          <table className="min-w-[1150px] w-full border-separate border-spacing-0 text-left text-xs">
            <thead>
              <tr className="text-muted-foreground">
                <th className="sticky top-0 z-20 w-[70px] whitespace-nowrap border-b border-border bg-[#090d1f] px-5 py-4"></th>
                <th className="sticky top-0 z-20 min-w-[120px] whitespace-nowrap border-b border-border bg-[#090d1f] px-5 py-4">
                  Emp ID
                </th>
                <th className="sticky top-0 z-20 min-w-[230px] whitespace-nowrap border-b border-border bg-[#090d1f] px-5 py-4">
                  Name
                </th>
                <th className="sticky top-0 z-20 min-w-[200px] whitespace-nowrap border-b border-border bg-[#090d1f] px-5 py-4">
                  Designation
                </th>
                <th className="sticky top-0 z-20 min-w-[190px] whitespace-nowrap border-b border-border bg-[#090d1f] px-5 py-4">
                  Department
                </th>
                <th className="sticky top-0 z-20 min-w-[260px] whitespace-nowrap border-b border-border bg-[#090d1f] px-5 py-4">
                  Email
                </th>
                <th className="sticky top-0 z-20 min-w-[180px] whitespace-nowrap border-b border-border bg-[#090d1f] px-5 py-4">
                  Contact
                </th>
              </tr>
            </thead>
              ...
              {filteredEmployees.map((employee) => {
                ...
                return (
                  <tr key={employee.id} className="hover:bg-muted/20">
                    ...
                    <td className="whitespace-nowrap border-b border-border/50 px-5 py-4 font-mono text-muted-foreground">
                      {employee.empId}
                    </td>

                    <td className="whitespace-nowrap border-b border-border/50 px-5 py-4">
                      <div className="flex items-center gap-3">
                        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-violet-500 text-xs font-bold text-white">
                          {employee.initials}
                        </div>
  ...
  return (
    <div className="overflow-auto rounded-xl border border-border">
      <table className="min-w-[1350px] w-full border-separate border-spacing-0 text-left text-xs">
        <thead>
          <tr className="text-muted-foreground">
            <th className="sticky top-0 z-20 w-[90px] whitespace-nowrap border-b border-border bg-[#090d1f] px-5 py-4">
              S.No.
            </th>
            <th className="sticky top-0 z-20 min-w-[180px] whitespace-nowrap border-b border-border bg-[#090d1f] px-5 py-4">
              Imported Name
      ...
      <table className="min-w-[1250px] w-full border-separate border-spacing-0 text-left text-xs">
        <thead>
          <tr className="text-muted-foreground">
            ...
            <th className="sticky top-0 z-20 min-w-[140px] whitespace-nowrap border-b border-border bg-[#090d1f] px-5 py-4">
              Status
            </th>
            <th className="sticky top-0 z-20 min-w-[180px] whitespace-nowrap border-b border-border bg-[#090d1f] px-5 py-4">
              Sync Action
            </th>
            <th className="sticky top-0 z-20 min-w-[380px] whitespace-nowrap border-b border-border bg-[#090d1f] px-5 py-4">
              Note
            </th>
          </tr>
        </thead>
      ...
      <table className="min-w-[1450px] w-full border-separate border-spacing-0 text-left text-xs">
        <thead>
          <tr className="text-muted-foreground">
            ...
            <th className="sticky top-0 z-20 min-w-[280px] whitespace-nowrap border-b border-border bg-[#090d1f] px-5 py-4">
              Reason
            </th>
          </tr>
        </thead>
        ...
        <tbody>
          {results.map((result) => (
            <tr key={result.id} className="hover:bg-muted/20">
              ...
              <td className="whitespace-nowrap border-b border-border/50 px-5 py-4">
                <span className="inline-flex whitespace-nowrap rounded-full border border-amber-500/20 bg-amber-500/10 px-3 py-1 text-[11px] font-bold text-amber-400">
                  {result.reason}
                </span>
              </td>
            </tr>
...
function WarningBox({ text }) {
  return (
    <div className="rounded-xl border border-amber-500/20 bg-amber-500/10 p-4">
      <p className="text-xs font-bold text-amber-400">{text}</p>
    </div>
  );
}
...
function SuccessBox({ text }) {
  return (
    <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/10 p-4">
      <p className="text-xs font-bold text-emerald-400">{text}</p>
    </div>
  );
}
...
function SmallPopup({ title, children, onClose }) {
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 px-4 backdrop-blur-md">
      <div className="w-full max-w-lg rounded-2xl border border-primary/30 bg-[#090d1f] p-6 shadow-2xl shadow-primary/20">
