...
export function MessagesModal({ isOpen, onClose }) {
    ...
    if (activeTab === "message") {
      ...
      Promise.all([
        api
          ...
          .catch(() => ({ data: [] })),
        api.get(`/api/emails?type=message${emailQuery}`).catch(() => ({ data: [] })),
      ])
      ...
      api
        .get(`/api/emails?type=${activeTab}${emailQuery}`)
            ...
            if (stored) {
              ...
              setCurrentUser({
                ...
                code:
                  ...
                  parsed.code ||
                  parsed.username ||
                  (parsed.role === "admin" ? "allchar" : "EMP"),
                role: parsed.role || "admin",
                email:
                  parsed.email ||
                  parsed.workEmail ||
                  parsed.username ||
                  (parsed.role === "admin" ? "allchar" : ""),
              });
            } else {
              ...
              setCurrentUser({
                ...
                code,
                role,
                email: localStorage.getItem("userEmail") || code,
              });
            }
          } catch (err) {
  ...
  useEffect(() => {
    ...
    if (selectedItem) {
      const selectedStillExists = selectedItem.isGroupedConversation
        ...
        : items.some((item) => isSameItem(item, selectedItem));
  ...
  const getCanonicalContactKey = (rawStr = "") => {
    if (!rawStr) return "unknown";

    const clean = cleanEmailAddress(rawStr).toLowerCase().trim();
    if (!clean) return "unknown";

    const isValid = (str) =>
      str && str !== "na" && str !== "n/a" && str !== "no email";

    const foundEmp = employees.find((emp) => {
      const code = String(emp.employeeCode || emp.code || "").toLowerCase().trim();
      const empNum = String(emp.employeeNumber || "").toLowerCase().trim();
      const cEmail = String(emp.work?.companyEmail || "").toLowerCase().trim();
      const pEmail = String(emp.personal?.contactEmail || "").toLowerCase().trim();
      const genEmail = String(emp.email || "").toLowerCase().trim();
      const fn = String(emp.personal?.firstName || "").toLowerCase().trim();
      const ln = String(
      ...
      return (
        (code && code === clean) ||
        (empNum && empNum === clean) ||
        (isValid(cEmail) && cEmail === clean) ||
        (isValid(pEmail) && pEmail === clean) ||
        (isValid(genEmail) && genEmail === clean) ||
    ...
    if (foundEmp) {
      return `EMP_${foundEmp.employeeCode || foundEmp._id}`;
    }

    return `EMAIL_${clean}`;
  };

  const resolveEmployeeName = (emailOrStr) => {
    if (!emailOrStr) return "Unknown";

    let clean = cleanEmailAddress(emailOrStr).trim();
    if (clean.startsWith("EMP_")) {
      clean = clean.replace("EMP_", "");
    } else if (clean.startsWith("EMAIL_")) {
      clean = clean.replace("EMAIL_", "");
    }
    ...
    const foundEmp = employees.find((emp) => {
      ...
      const genEmail = String(emp.email || "").toLowerCase().trim();
      return (
        (code && code === lowerClean) ||
        (empNum && empNum === lowerClean) ||
        (isValid(cEmail) && cEmail === lowerClean) ||
        (isValid(pEmail) && pEmail === lowerClean) ||
        (isValid(genEmail) && genEmail === lowerClean)
      );
    });

    if (foundEmp) {
      const fn = foundEmp.personal?.firstName || "";
      const ln =
        foundEmp.personal?.lastName && foundEmp.personal?.lastName !== "Emp"
          ? foundEmp.personal?.lastName
          : "";
      const fullName = `${fn} ${ln}`.trim();
      if (fullName) return fullName;
      if (foundEmp.name) return foundEmp.name;
    }

    if (
      currentUser?.email &&
      (lowerClean === String(currentUser.email).toLowerCase() ||
        lowerClean === String(currentUser.code || "").toLowerCase())
    ) {
      return currentUser.name || "Admin";
    }

    if (emailOrStr.includes("<") && emailOrStr.includes(">")) {
      const namePart = emailOrStr.split("<")[0].trim();
  ...
  const getDisplayPerson = (item) => {
    if (item?.isGroupedConversation) {
      return item.partnerName || resolveEmployeeName(item.partnerEmail);
    }
    ...
    const targetEmail = sentByMe
      ? item?.to || ""
      : item?.from || item?.to || "";

    return resolveEmployeeName(targetEmail);
  };
  ...
  const getSenderEmail = (item) => {
    ...
    return item.senderEmail || item.to || "Unknown";
  };
  ...
  const isSentByCurrentUser = (item) => {
    ...
    if (myEmail && fromStr.includes(myEmail)) return true;
    if (myName && fromStr.includes(myName)) return true;
    if (myCode && fromStr.includes(myCode)) return true;
  ...
  const getPartnerEmailKey = (item) => {
    ...
    if (item.isChatConversation) {
      const partnerStr =
        ...
        item.from ||
        "unknown";
      return getCanonicalContactKey(partnerStr);
    }
    const sentByMe = isSentByCurrentUser(item);
    const rawEmail = sentByMe ? item.to || "" : item.from || item.to || "";
    return getCanonicalContactKey(rawEmail);
  };
  ...
  const getSortedInboxItems = () => {
    ...
    items.forEach((item) => {
      const key = getPartnerEmailKey(item);

      if (!groupsMap[key]) {
        const partnerName = resolveEmployeeName(key);
        groupsMap[key] = {
          _id: `group_${key}`,
          isGroupedConversation: true,
          partnerKey: key,
          partnerEmail: key,
      ...
      groupsMap[key].emails.push(item);

      const currentLatestTime = new Date(
        groupsMap[key].latestEmail?.createdAt || 0
      ).getTime();
      const itemTime = new Date(item.createdAt || 0).getTime();
      if (itemTime > currentLatestTime) {
        groupsMap[key].latestEmail = item;
        groupsMap[key].createdAt = item.createdAt;
      }

      if (!item.isRead) {
        groupsMap[key].unreadCount += 1;
      }
    });

    return Object.values(groupsMap)
      .map((group) => {
        group.isRead = group.unreadCount === 0;
        group.subject = group.latestEmail?.subject || "No Subject";
        group.preview = getMessagePreview(group.latestEmail);
        group.from = group.latestEmail?.from;
        group.to = group.latestEmail?.to;
        return group;
      })
      .sort((a, b) => {
        const first = new Date(a.createdAt || 0).getTime();
  ...
  const getConversationItemsForSelected = () => {
    ...
    if (selectedItem.isGroupedConversation && selectedItem.emails) {
      return [...selectedItem.emails].sort((a, b) => {
        const first = new Date(a.createdAt || 0).getTime();
        const second = new Date(b.createdAt || 0).getTime();
    ...
    const targetKey = getPartnerEmailKey(selectedItem);

    const related = items.filter(
      (item) => getPartnerEmailKey(item) === targetKey
    );

    const list = related.length > 0 ? related : [selectedItem];
  ...
  const getReplyRecipientEmail = (item) => {
    ...
    if (item.isGroupedConversation) {
      ...
      if (item.latestEmail) {
        return isSentByCurrentUser(item.latestEmail)
          ? item.latestEmail.to || ""
  ...
  const markItemAsRead = async (item) => {
    ...
    if (item.isGroupedConversation) {
      const partnerKey = item.partnerKey;
      setItems((prev) =>
        prev.map((i) =>
          getPartnerEmailKey(i) === partnerKey
      ...
      if (item.emails && item.emails.length > 0) {
        const unreadIds = item.emails
          .filter((e) => !e.isRead && e._id)
          .map((e) => e._id);
        await Promise.all(
          unreadIds.map((id) =>
            api.patch(`/api/emails/${id}/read`).catch(() => {})
          )
        );
      }
      return;
    ...
    if (item.isChatConversation) {
      ...
      try {
        await api.patch(`/api/emails/${item._id}/read`).catch(() => {});
      } catch (e) {
  ...
  const handleMarkAllAsRead = async () => {
    ...
    if (activeTab === "email") {
      ...
      const emailQuery = uEmail ? `?userEmail=${encodeURIComponent(uEmail)}&type=email` : "?type=email";
      await api.patch(`/api/emails/mark-all-read${emailQuery}`).catch(() => {});
    } else {
      const uId = currentUser?.id || "";
      const uCode = currentUser?.code || "";
      const uRole = currentUser?.role || "";
  ...
  const renderConversationBubble = (item, idx) => {
    const isMyMessage = isSentByCurrentUser(item);
    const isUnread =
        ...
        >
          <div
            className={`rounded-2xl border p-2.5 shadow-sm transition-all ${
              isMyMessage
                ? "border-purple-300 bg-purple-100/90 dark:border-indigo-500/40 dark:bg-indigo-600/15"
                : "border-blue-300 bg-blue-100/90 dark:border-slate-700 dark:bg-slate-800/70"
            }`}
          >
            <div
              className={`rounded-xl border px-3.5 py-2 text-xs font-bold ${
                isMyMessage
                  ? "border-purple-300 bg-purple-200/80 text-[#260b45] dark:border-indigo-500/30 dark:bg-indigo-500/15 dark:text-indigo-100"
                  : "border-blue-300 bg-blue-200/80 text-blue-950 dark:border-slate-700 dark:bg-slate-900/80 dark:text-slate-100"
              }`}
            >
              <div className="flex items-center justify-between gap-2">
                <span className="line-clamp-2">
                  Subject: {getMessageTitle(item)}
                </span>
            ...
            <div
              className={`mt-2 rounded-xl border px-3.5 py-3 text-xs leading-relaxed ${
                isMyMessage
                  ? "border-purple-200 bg-white text-[#260b45] dark:border-indigo-500/20 dark:bg-indigo-500/10 dark:text-slate-100"
                  : "border-blue-200 bg-white text-slate-900 dark:border-slate-700 dark:bg-slate-950/50 dark:text-slate-100"
              }`}
            >
            ...
            <div className="mt-2 flex items-center justify-between gap-2 px-1">
              ...
              <span className="shrink-0 text-[10px] font-medium text-muted-foreground">
                {isMyMessage ? "Sent" : "Received"} on {formatDate(item.createdAt)}
              </span>
            </div>
